...
pub fn execute_add_integer_instruction(cpu: &mut MapleCPU, args: &InstructionArguments) {
    let value_a = resolve_potential_register_argument_value(cpu, args.arg1_raw as u64);
    let value_b = resolve_potential_register_argument_value(cpu, args.arg2_raw as u64);

    let (result, overflowed) = value_a.overflowing_add(value_b);

    cpu.set_register(args.rdest, value_a + value_b);
}
...
fn update_conditional_result_register(cpu: &mut MapleCPU, operation_result: u64, overflowed: bool) {
    let overflow = overflowed as u64;
    let zero = ((operation_result == 0) as u64) << 1;
    let negative = extract_from_binary_left(operation_result, 1, 0) << 2; // MSB bit
