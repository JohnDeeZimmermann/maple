...
pub fn execute_move_instruction(cpu: &mut MapleCPU, memory: &mut Memory, instruction: u64) {
    let opt: u64 = extract_from_binary_left(instruction, 1, 8);
    let rdest: u64 = extract_from_binary_left(instruction, 4, 9);
    let raw_value: u64 = extract_from_binary_right(instruction, 51, 0);
    let is_move_not = opt == 1;

    let actual_value: u32 = resolve_potential_register_argument_value(cpu, raw_value) as u32;

    if is_move_not {
        memory.write(!actual_value, cpu);
    } else {
        memory.write(actual_value, cpu);
    }
}
